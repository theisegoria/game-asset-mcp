...
1. **No vendor type escapes the provider layer.** Tripo's `{ code, data, message }` envelope, Leonardo's `PENDING`/`COMPLETE`/`FAILED` vocabulary, their differing id schemes — all of it is normalized at the boundary, and no vendor *type* appears in a domain signature.
   ...
   Two honest exceptions, since the stronger claim — that nothing above the layer imports a vendor module at all — is not true. `tools/context.ts` constructs the concrete providers, which is what a composition root is for. And several tools import vendor constants: `tools/assets3d.ts` interpolates `TRIPO_DEFAULT_MODEL_VERSION` into an advertised parameter description, and `tools/audio.ts` does the same with Leonardo's duration bounds. `tools/animation.ts` and `tools/textures.ts` import the constant without putting it in a schema. Those interpolations are genuine leaks of vendor detail into the MCP surface, and they are where this invariant is currently bent — not one place, several.
2. **Nothing is lost in normalization.** Every mapped value keeps its raw counterpart beside it: `AssetJob.providerStatus` holds the provider's own string next to our `status`, and `Model3DTaskResult.raw` carries the entire untouched payload. Honest caveat: `raw` is populated by every provider and currently read by nothing — it is available to a debugger holding the object, not surfaced through any tool. Normalization that discards the original just relocates the debugging problem to a place where you no longer have the evidence.
